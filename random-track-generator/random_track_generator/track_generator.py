...
def _create_track(n_points: int, 
    ...
    while True:
        ...
        # Sort vertices
        # sorted_vertices = np.array([[0, 0], [90, 0], [100, 0], [100, 10], [100, 90], [100, 100], [90, 100], [10, 100], [0, 100], [0, 90]])

        # sorted_vertices = np.array([[0, 0], [200, 0], [200, 100], [0, 100]])
        ...
        centreline_points = np.column_stack([x, y])
        track = Polygon(centreline_points)
    ...
    cone_spacing_left = np.linspace(0, track_left.length, np.ceil(track_left.length / CONE_SPACING).astype(int) + 1)[:-1]
    cone_spacing_right= np.linspace(0, track_right.length, np.ceil(track_right.length / CONE_SPACING).astype(int) + 1)[:-1]
    ...
    M_rt  = transformation_matrix(-start_position, start_heading - np.pi/2)
    ...
    centreline_points = M.dot(np.c_[centreline_points, np.ones(len(centreline_points))].T)[:-1].T
    # centreline_points = np.column_stack([x, y])
    
        
    reference_point = np.array([0,0])
    distances = np.linalg.norm(centreline_points - reference_point, axis=1)
    closest_idx = np.argmin(distances)
    centreline_points = np.roll(centreline_points, -closest_idx, axis=0)
    
    track = Track(cones_left, cones_right, centreline_points)
    # track.orange_cones = M.dot(np.c_[track.orange_cones, np.ones(len(track.orange_cones))].T)[:-1].T
